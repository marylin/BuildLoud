...
let CACHE_PATH = join(homedir(), '.claude', 'journey', 'cache.json');
...
function acquireLock() {
  ...
  logError('cache-lock', 'Could not acquire lock after 3 retries, proceeding anyway');
  return false;
}

function releaseLock() {
  try { unlinkSync(CACHE_PATH + '.lock'); } catch { /* ok */ }
}
