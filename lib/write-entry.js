...
let opts = { skipGit: false, skipSupabase: false, skipSeo: false };
...
export async function write(entry, date = new Date()) {
  ...
  // 5. Write to Supabase (async, non-blocking)
  if (!opts.skipSupabase) {
    supabase.insert(entry).catch(() => {}); // fire-and-forget, queue handles failures
  }
