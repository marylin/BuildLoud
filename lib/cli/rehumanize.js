...
export async function run(args) {
  ...
  if (values.id) {
    candidates = await sql`
      ...
      WHERE id = ${values.id} AND social_score >= 5
        AND (metadata->>'humanize_pending' = 'true')
    `;
  } else if (values.backfill) {
    candidates = await sql`
      SELECT * FROM journey_entries
      WHERE social_score >= 5
        AND (metadata->>'public_summary' IS NULL)
        AND (metadata->>'humanize_pending' IS NULL)
      ORDER BY social_score DESC, created_at DESC
    `;
  } else {
    candidates = await sql`
      SELECT * FROM journey_entries
      WHERE metadata->>'humanize_pending' = 'true'
      ORDER BY social_score DESC, created_at DESC
    `;
  }
  ...
  for (const row of candidates) {
    try {
      ...
      const merged = { ...(row.metadata || {}) };
      delete merged.humanize_pending;
