...
export function processSession(filePath) {
  ...
  for (const [project, projectEvents] of Object.entries(byProject)) {
    ...
    if (projectCommits.length === 0) continue; // Notable-only sessions need commits for context
...
export async function run(args) {
  const { values } = parseArgs({
    ...
    options: {
      ...
      'output-pending': { type: 'string' },
    },
  });
  if (!values.file) {
    console.error('Usage: journey process-session --file PATH [--output-pending PATH]');
    process.exit(1);
  }
  const results = processSession(values.file);
  console.log(JSON.stringify(results, null, 2));

  const pendingPath = values['output-pending'];
  if (pendingPath) {
    const highScoreEntries = results.filter(r => r.score >= 5);
